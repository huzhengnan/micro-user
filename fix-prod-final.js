...
async function fixProductionData() {
  ...
  try {
    ...
    // 首先清理所有现有的Creem映射
    console.log('\n清理现有Creem映射...');
    const deleteResult = await freshPrisma.paymentProductMapping.deleteMany({
    ...
    for (const planData of correctPlans) {
      ...
      // 查找现有计划
      let plan = await freshPrisma.subscriptionPlan.findFirst({
        where: { name: planData.name }
      });
      
      if (plan) {
        // 更新现有计划
        plan = await freshPrisma.subscriptionPlan.update({
          where: { id: plan.id },
          data: {
        ...
        // 创建新计划
        plan = await freshPrisma.subscriptionPlan.create({
          data: {
            name: planData.name,
            description: planData.description,
            price: planData.price,
      ...
      // 创建正确的Creem映射
      await freshPrisma.paymentProductMapping.create({
        data: {
    ...
    // 验证修复结果
    console.log('\n=== 验证修复结果 ===');
    const allPlans = await freshPrisma.subscriptionPlan.findMany({
      include: {
        paymentMappings: {
          where: { paymentProvider: 'CREEM' }
        }
